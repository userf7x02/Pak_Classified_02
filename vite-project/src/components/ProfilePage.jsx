...
export default function ProfilePage({ isAuthenticated, setIsAuthenticated }) {
  ...
  const handleDeleteAd = async (adId) => {
    ...
    try {
      await fetch(`https://pak-backend.vercel.app/createAdvertisement/delete/${adId}`, { method: "DELETE" });
  ...
  const EditAdDone = async (data) => {
    try {
      ...
      const res = await fetch(
        `https://pak-backend.vercel.app/createAdvertisement/update/${currentAd._id}`,
        {
          method: "PUT",
          body: formData,
  ...
  const EditDone = async (data) => {
    ...
    try {
      ...
      const res = await fetch(`https://pak-backend.vercel.app/createuser/update/${user._id}`, {
        method: "PUT",
        body: formData,
      ...
      setUser({
        ...
        image: updatedUser.image.startsWith("/uploads")
          ? `https://pak-backend.vercel.app${updatedUser.image}`
          : updatedUser.image
  ...
  useEffect(() => {
    fetch("https://pak-backend.vercel.app/createCategory/Get", {
    ...
    fetch("https://pak-backend.vercel.app/createArea/Get", {
  ...
  ? userData.image.startsWith("http")
    ? userData.image
    : `https://pak-backend.vercel.app/uploads/${userData.image.replace(/^\/uploads\//, "")}`
  ...
  useEffect(() => {
  if (user?._id) {
    fetch(`https://pak-backend.vercel.app/createAdvertisement/user/${user._id}`)
              ...
              <div key={ad._id} className="ad-card">
                <img
                  src={ad.Image?.startsWith("http")
                    ? ad.Image
                    : `https://pak-backend.vercel.app/uploads/${ad.Image}`}
                  alt={ad.Name}
