...
export default function ProfilePage({ isAuthenticated, setIsAuthenticated }) {
  ...
  const handleDeleteAd = async (adId) => {
    ...
    try {
      await fetch(`${VITE_API_URL}/createAdvertisement/delete/${adId}`, { method: "DELETE" });
  ...
  const EditAdDone = async (data) => {
    try {
      ...
      const res = await fetch(
        `${VITE_API_URL}/createAdvertisement/update/${currentAd._id}`,
        {
          method: "PUT",
          body: formData,
  ...
  const EditDone = async (data) => {
    ...
    try {
      ...
      const res = await fetch(`${VITE_API_URL}/createuser/update/${user._id}`, {
        method: "PUT",
        body: formData,
      ...
      setUser({
        ...
        image: updatedUser.image.startsWith("/uploads")
          ? `${VITE_API_URL}${updatedUser.image}`
          : updatedUser.image
  ...
  useEffect(() => {
  if (user?._id) {
    fetch(`${VITE_API_URL}/createAdvertisement/user/${user._id}`)
